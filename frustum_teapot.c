...
// ! Para almacenar los id de las ventanas.
int window_view;
int window_observer;
...
/*
	Para dibujar el frustum, usando los parámetros:
	-left
	-right
	-bottom
	-top
...
static void draw_grid1d(
	double ax, double ay, double az,  /* avance en el grid */
	double dx, double dy, double dz,  /* dirección del grid */
	float l0, float l1,            /* rango de línea */
	int major, int begin, int end, /* línea principal, inicio y fin del grid */
	float or, float og, float ob   /* color original r,g,b */ )
{
	if( begin > end ) {
		int t = begin;
		begin = end;
...
/*
	Se encarga de la iluminación, y de dibujar la tetera
	y el cubo.
*/
{
	glPushMatrix();
...
void display_view(int const win_width, int const win_height)
{
	...
	glutSwapBuffers();//Intercambio de buffers, para mostrar el renderizado
}

void display(void) //Se va a ejecutar cada que se actualice la pantalla
{
	glutSetWindow(window_view);//Estableciendo ventana de vista
	...
	display_view(win_view_width, win_view_height);//Perspectiva vista (fija)

	glutSetWindow(window_observer);//Estableciendo ventana observador
	display_observer(win_view_aspect);//perspectiva observador
}
